...
import com.android.simplechat.web_rtc.WebSocketRTCClient;
...
import static com.android.simplechat.web_rtc.utils.Constants.CAPTURE_PERMISSION_REQUEST_CODE;
import static com.android.simplechat.web_rtc.utils.Constants.EXTRA_ROOMID;
import static com.android.simplechat.web_rtc.utils.Constants.LOCAL_HEIGHT_CONNECTED;
import static com.android.simplechat.web_rtc.utils.Constants.LOCAL_HEIGHT_CONNECTING;
import static com.android.simplechat.web_rtc.utils.Constants.LOCAL_WIDTH_CONNECTED;
import static com.android.simplechat.web_rtc.utils.Constants.LOCAL_WIDTH_CONNECTING;
import static com.android.simplechat.web_rtc.utils.Constants.LOCAL_X_CONNECTED;
import static com.android.simplechat.web_rtc.utils.Constants.LOCAL_X_CONNECTING;
import static com.android.simplechat.web_rtc.utils.Constants.LOCAL_Y_CONNECTED;
import static com.android.simplechat.web_rtc.utils.Constants.LOCAL_Y_CONNECTING;
import static com.android.simplechat.web_rtc.utils.Constants.REMOTE_HEIGHT;
import static com.android.simplechat.web_rtc.utils.Constants.REMOTE_WIDTH;
import static com.android.simplechat.web_rtc.utils.Constants.REMOTE_X;
import static com.android.simplechat.web_rtc.utils.Constants.REMOTE_Y;
import static com.android.simplechat.web_rtc.utils.Constants.STAT_CALLBACK_PERIOD;
...
public class CallActivity extends AppCompatActivity
    ...
    private String recFirebaseToken;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (callType.equals(Constants.OUTGOING_CALL)) {
            binding.rlCallLayout.setVisibility(View.GONE);
            connect();
        } else {
            vibrate();
            connect();
            binding.callLayout.txtWho.setText(sender);
            binding.callLayout.imgAccept.setOnClickListener((view -> {
                stopVibrating();
                binding.rlCallLayout.setVisibility(View.GONE);
            }));
            binding.callLayout.imgReject.setOnClickListener((view -> {
                disconnect();
                stopVibrating();
                finish();
            }));
    ...
    private void connect() {
        ...
        if (EasyPermissions.hasPermissions(this, perms)) {
            executeCallProcedure();
        } else {
            EasyPermissions.requestPermissions(this, "Need some permissions", RC_CALL, perms);
        }
    }

    private void executeCallProcedure() {
    ...
    private void getExtras() {
        ...
        if (b != null) {
            ...
            sender = getIntent().getExtras().getString(Constants.ARG_SENDER);
            receiver = getIntent().getExtras().getString(Constants.ARG_RECEIVER);
            senderUid = getIntent().getExtras().getString(Constants.ARG_SENDERUID);
            recFirebaseToken = getIntent().getExtras().getString(Constants.ARG_RECEIVER_FIREBASE_TOKEN);
            firebasetoken = getIntent().getExtras().getString(Constants.ARG_FIREBASE_TOKEN);
            type = getIntent().getExtras().getString(Constants.ARG_TYPE);
            callType = getIntent().getExtras().getString(Constants.CALL_TYPE);
        }
    }
    ...
    private void onConnectedToRoomInternal(final AppRTCClient.SignalingParameters params) {
        ...
        if (signalingParameters.initiator) {
            ...
            //send a push notification to the callee to accept
            sendPushNotificationToReceiver(sender, "", senderUid, firebasetoken, recFirebaseToken, type);
    ...
    private void sendPushNotificationToReceiver(String username, String message, String uid, String firebaseToken,
                                                String receiverFirebaseToken, String type) {
